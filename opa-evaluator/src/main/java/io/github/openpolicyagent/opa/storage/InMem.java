  ...
  protected void writeData(String bundleId, String root, RegoObject value) {
    ...
    if (root.isEmpty()) {
      ...
      for (int i = 0; i < pathParts.length - 1; i++) {
        io.github.openpolicyagent.opa.ast.types.RegoString key =
            new io.github.openpolicyagent.opa.ast.types.RegoString(pathParts[i]);
        io.github.openpolicyagent.opa.ast.types.RegoValue existing = current.getProperty(key);
      ...
      io.github.openpolicyagent.opa.ast.types.RegoString finalKey =
          new io.github.openpolicyagent.opa.ast.types.RegoString(pathParts[pathParts.length - 1]);
      io.github.openpolicyagent.opa.ast.types.RegoValue existingValue = current.getProperty(finalKey);
